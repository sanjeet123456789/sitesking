...
if(isset($_GET['offset'])&&isset($_GET['limit'])){
	...
	$data=mysqli_query($connection,"
		...
		SELECT *  from community where parent_id between 800 and   899 and follower !=0 order by follower desc LIMIT {$limit} OFFSET {$offset}");
...
//    ';
	...
	while($row=mysqli_fetch_array($data)){
		...
		$pic=$row['pic_link'];
		$index=$row['position'];
		$pic_link_youtube="assets/images/icons/youtube.png";



		echo '<li>'.$i.' <a class="web_link" target="_blank" href="'.$row['link'].'" ><img   src="'.$pic_link_youtube.'"  class="web_icon" >y/'.$row['name'].'  </a><span class="follower">'.$follow.'</span></li>';$i++;
