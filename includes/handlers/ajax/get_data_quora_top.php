...
if(isset($_GET['offset'])&&isset($_GET['limit'])){
	...
	$data=mysqli_query($connection,"
		...
		SELECT *  from community where parent_id between 200 and   299 and follower !=0 order by follower desc LIMIT {$limit} OFFSET {$offset}");
...
//    ';
	...
	while($row=mysqli_fetch_array($data)){
		...
		$pic=$row['pic_link'];
		$pic_link_quora="assets/images/icons/quora.png";
		
		



		echo '<li>'.$i.' <a class="web_link" target="_blank" href="'.$row['link'].'" ><img   src="'.$pic_link_quora.'"  class="web_icon" >q/'.$row['name'].'  </a><span class="follower">'.$follow.'</span></li>';$i++;
